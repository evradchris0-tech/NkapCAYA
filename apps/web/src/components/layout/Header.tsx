...
function avatarColor(_initials: string): string {
  // Palette resserrée : avatar marine uniforme (design system NkapZen)
  return 'bg-primary-700 text-white';
}

const ROLE_BADGE_COLORS: Partial<Record<BureauRole, string>> = {
  [BureauRole.SUPER_ADMIN]:             'bg-accent-100 text-accent-800',
  [BureauRole.PRESIDENT]:               'bg-primary-100 text-primary-800',
  [BureauRole.VICE_PRESIDENT]:          'bg-primary-100 text-primary-700',
  [BureauRole.TRESORIER]:               'bg-primary-50 text-primary-700',
  [BureauRole.SECRETAIRE_GENERAL]:      'bg-primary-50 text-primary-700',
  [BureauRole.SECRETAIRE_ADJOINT]:      'bg-primary-50 text-primary-600',
  [BureauRole.COMMISSAIRE_AUX_COMPTES]: 'bg-slate-100 text-slate-700',
  [BureauRole.MEMBRE]:                  'bg-slate-100 text-slate-500',
};

const STATUS_LABELS: Record<string, { label: string; cls: string }> = {
  ACTIVE:    { label: 'ACTIF',      cls: 'bg-emerald-100 text-emerald-700' },
  PENDING:   { label: 'EN ATTENTE', cls: 'bg-amber-100 text-amber-700' },
  CASSATION: { label: 'CASSATION',  cls: 'bg-amber-100 text-amber-800' },
  CLOSED:    { label: 'CLÔTURÉ',    cls: 'bg-slate-100 text-slate-600' },
  ARCHIVED:  { label: 'ARCHIVÉ',    cls: 'bg-slate-100 text-slate-400' },
};
...
export default function Header() {
  ...
  const roleBadgeCls = user?.role ? (ROLE_BADGE_COLORS[user.role as BureauRole] ?? 'bg-slate-100 text-slate-600') : '';

  // Computed FY descriptive info
  const fyStatus = selectedFy ? STATUS_LABELS[selectedFy.status] ?? { label: selectedFy.status, cls: 'bg-slate-100 text-slate-600' } : null;
    ...
    <>
      <header className="h-14 md:h-16 shrink-0 border-b border-slate-200 flex items-center justify-between px-4 md:px-6 shadow-sm gap-3 sticky top-0 z-40 bg-white/80 backdrop-blur-md">
        ...
        <div className="flex items-center gap-2 md:gap-3 min-w-0 flex-1">
            ...
            <div className="flex items-center gap-2 min-w-0">
              <div className="relative" ref={fyDropdownRef}>
                ...
                >
                  ...
                  <ChevronDown className={`h-3.5 w-3.5 text-slate-400 shrink-0 transition-transform duration-150 ${showFyDropdown ? 'rotate-180' : ''}`} />
                </button>

                {/* Dropdown list */}
                {showFyDropdown && (
                  <div className="absolute left-0 top-full mt-1.5 w-64 bg-white rounded-xl border border-slate-200 shadow-lg z-50 overflow-hidden py-1">
                    <p className="px-3 pt-2 pb-1 text-[10px] font-semibold text-slate-400 uppercase tracking-wider">
                      Exercices fiscaux
                    </p>
                    <ul role="listbox">
                      {fiscalYears.map((fy) => {
                        const st = STATUS_LABELS[fy.status] ?? { label: fy.status, cls: 'bg-slate-100 text-slate-600' };
                        ...
                        return (
                          <li key={fy.id} role="option" aria-selected={isSelected}>
                            ...
                            >
                              <div className={`w-2 h-2 rounded-full shrink-0 ${isSelected ? 'bg-primary-500' : 'bg-transparent border border-slate-300'}`} />
                              <div className="flex-1 min-w-0">
                                <p className={`text-xs font-semibold truncate ${isSelected ? 'text-primary-700' : 'text-slate-800'}`}>
                                  {fy.label}
                                </p>
              ...
              {/* Notes de l'exercice */}
              {selectedFy?.notes && (
                <span className="hidden lg:inline text-xs text-slate-400 italic truncate max-w-[180px]">
                  {selectedFy.notes}
                </span>
              )}
            </div>
          ...
          {!canSwitchFy && selectedFy && (
            <div className="hidden sm:flex items-center gap-2 text-xs text-slate-500">
              <span className="text-slate-600 font-medium">{selectedFy.label}</span>
              {fyStatus && (
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          {/* Date du jour */}
          <span className="hidden md:inline-flex items-center text-xs text-slate-400 font-medium capitalize px-2 py-1 bg-slate-50 rounded-lg border border-slate-100 shrink-0">
            {todayLabel}
          </span>
                ...
                {visibleNotifications.length === 0 ? (
                  ...
                  <ul className="divide-y divide-slate-100 max-h-72 overflow-y-auto">
                    {visibleNotifications.map((notif) => {
                      const content = (
                        <div className="flex items-start gap-3 px-4 py-3 hover:bg-slate-50 transition-colors cursor-pointer group/notif">
                          ...
                          <div className="flex items-center gap-1 shrink-0">
                            ...
                            <button
                              ...
                              className="opacity-0 group-hover/notif:opacity-100 transition-opacity p-0.5 rounded text-slate-300 hover:text-slate-500 hover:bg-slate-100"
                            >
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-sm text-slate-400 hover:text-red-500 px-2 py-1.5 rounded-lg hover:bg-red-50 transition-colors"
          >
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 flex flex-col gap-5">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-base font-semibold text-slate-900">Changement d&apos;exercice</h2>
                <p className="text-xs text-slate-500 mt-0.5">Authentification requise</p>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-medium text-slate-700">Code d&apos;accès</label>
              <input
                ref={codeInputRef}
                type="password"
                placeholder="••••••••"
                ...
                className="w-full px-3 py-2.5 text-sm border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-transparent tracking-widest"
                autoComplete="off"
              />
              {codeError && (
