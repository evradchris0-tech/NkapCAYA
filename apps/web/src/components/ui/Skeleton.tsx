    ...
    <tr>
      {Array.from({ length: cols }).map((_, i) => (
        <td key={i} className="px-4 py-3.5">
          <div
            className="h-4 animate-pulse bg-slate-200 rounded-md"
...
export function SkeletonCard() {
  return (
    <div className="bg-white rounded-xl p-6 border border-slate-100 shadow-card">
      <div className="flex items-start gap-4">
        <div className="h-10 w-10 animate-pulse bg-slate-200 rounded-lg shrink-0" />
        <div className="flex-1 space-y-2 pt-0.5">
          <div className="h-3 w-24 animate-pulse bg-slate-200 rounded" />
          <div className="h-7 w-20 animate-pulse bg-slate-300 rounded" />
        </div>
      </div>
    </div>
  );
