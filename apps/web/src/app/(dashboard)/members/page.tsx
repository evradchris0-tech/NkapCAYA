...
export default function MembersPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {/* Filtres */}
      <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-card flex flex-col md:flex-row gap-3 items-center">
        <div className="relative flex-1 w-full">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400 pointer-events-none" />
          <input
            type="text"
            placeholder="Rechercher (Nom, Code, Tél…)"
            className="w-full pl-9 pr-4 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-primary-500 focus:border-transparent"
            value={searchInput}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm min-w-[700px]">
              <thead className="bg-slate-50/80 border-b border-slate-200">
                <tr>
                  <th className="text-left px-4 py-3 font-semibold text-slate-500 uppercase tracking-wide text-xs w-10">#</th>
                  <th className="text-left px-4 py-3 font-semibold text-slate-500 uppercase tracking-wide text-xs">Code</th>
                  <th className="text-left px-4 py-3 font-semibold text-slate-500 uppercase tracking-wide text-xs">Nom complet</th>
                  <th className="text-left px-4 py-3 font-semibold text-slate-500 uppercase tracking-wide text-xs">Téléphone</th>
                  <th className="text-left px-4 py-3 font-semibold text-slate-500 uppercase tracking-wide text-xs">Quartier</th>
                  <th className="text-left px-4 py-3 font-semibold text-slate-500 uppercase tracking-wide text-xs">Rôle</th>
                  <th className="text-left px-4 py-3 font-semibold text-slate-500 uppercase tracking-wide text-xs">Statut</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {isLoading ? (
                  ...
                  data.data.map((member, index) => (
                    <tr key={member.id} className={`hover:bg-slate-50/60 transition-colors duration-100 ${!member.user.isActive ? 'opacity-50 bg-slate-50/40' : ''}`}>
                      <td className="px-4 py-3 text-slate-400 text-xs tabular-nums">{(page - 1) * LIMIT + index + 1}</td>
                      <td className="px-4 py-3 font-mono text-slate-500 text-xs">{member.memberCode}</td>
                      ...
                      <td className="px-4 py-3 text-slate-600">{member.phone1}</td>
                      <td className="px-4 py-3 text-slate-600">{member.neighborhood}</td>
